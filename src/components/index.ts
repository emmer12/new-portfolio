...
export const Text = styled.span<{
  ...
  line-height: 160%;
  color: ${({ color }) => (color ? color : "#8d98bd")};
...
export const SizeIcon = styled.div<{ height: number; width: number }>`
  svg {
    height: ${({ height }) => height + "px"};
...
export const FancyBg = styled.div<{ top: number; left: number }>`
  position: fixed;
  width: 87.5px;
  height: 47px;
  top: ${({ top }) => top && top + "%"};
